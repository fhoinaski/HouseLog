...
export default function NewPropertyPage() {
  ...
  return (
    <div className="safe-top safe-bottom min-h-screen overflow-x-hidden bg-hl-bg">
      <header className="fixed left-0 top-0 z-40 flex h-13 w-full items-center justify-between border-b border-hl-border bg-hl-surface px-4">
        <Button variant="ghost" size="icon" aria-label="Voltar" className="-ml-1" onClick={() => router.back()}>
          <ArrowLeft className="h-5 w-5" />
        </Button>
        <h1 className="pr-10 text-center text-md font-medium tracking-tight text-hl-text">Novo imovel</h1>
      </header>

      <main className="mx-auto mt-4 flex w-full max-w-3xl flex-col gap-4 px-4 pb-8">
        <div className="flex flex-col gap-2">
          <h2 className="text-xl font-medium tracking-tight text-hl-text">Adicionar propriedade</h2>
          <p className="max-w-lg text-sm leading-relaxed text-hl-text-muted">
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
          <button
            type="button"
            onClick={onPickCover}
            className="hl-btn-ghost group relative flex min-h-55 w-full cursor-pointer flex-col items-center justify-center overflow-hidden rounded-xl border border-hl-border bg-hl-surface p-6"
          ...
          >
            ...
            <div className={cn('z-10 flex flex-col items-center gap-3 text-center', coverPreview ? 'rounded-xl bg-hl-surface px-4 py-3' : '')}>
              <div className="mb-2 flex h-12 w-12 items-center justify-center rounded-full bg-hl-surface-muted text-hl-primary">
                <ImagePlus className="h-8 w-8" />
              </div>
              <h3 className="text-md font-medium text-hl-text">Imagem de capa</h3>
              <p className="max-w-55 text-xs leading-tight text-hl-text-muted">
          ...
          <Card className="p-1">
            <CardContent className="flex flex-col gap-4 p-0">
              <div className="rounded-lg bg-hl-surface p-4 transition-colors focus-within:bg-hl-surface-muted">
                <label htmlFor="name" className="hl-label mb-2 text-hl-text-muted">Nome da propriedade</label>
              ...
              <div className="rounded-lg bg-hl-surface p-4 transition-colors focus-within:bg-hl-surface-muted">
                <label htmlFor="address" className="hl-label mb-2 text-hl-text-muted">Endereco completo</label>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="relative rounded-lg bg-hl-surface p-4 transition-colors focus-within:bg-hl-surface-muted">
                  <label className="hl-label mb-2 text-hl-text-muted">Tipo de imovel</label>
                  <Select defaultValue="house" onValueChange={(value) => setValue('type', value as FormData['type'])}>
                    <SelectTrigger className="border-0 bg-transparent p-0 text-hl-text">
                  ...
                  <p className="mt-2 text-xs text-hl-text-muted">Selecionado: {typeLabel}</p>
                </div>

                <div className="rounded-lg bg-hl-surface p-4 transition-colors focus-within:bg-hl-surface-muted">
                  <label htmlFor="area_m2" className="hl-label mb-2 text-hl-text-muted">Area total (m2)</label>
                  <div className="flex items-center gap-2">
                    <Input id="area_m2" type="number" step="0.1" placeholder="0" className="border-0 bg-transparent p-0" {...register('area_m2')} />
                    <span className="text-sm text-hl-text-muted">m2</span>
                  </div>
                </div>
              </div>

              <div className="rounded-lg bg-hl-surface p-4 transition-colors focus-within:bg-hl-surface-muted">
                <label htmlFor="city" className="hl-label mb-2 text-hl-text-muted">Cidade</label>
          ...
          {error && (
            <div className="rounded-lg border border-[var(--hl-danger)] bg-[color-mix(in_srgb,var(--hl-danger)_12%,var(--hl-surface))] px-4 py-3 text-sm text-hl-danger">
              {error}
            </div>
          )}
