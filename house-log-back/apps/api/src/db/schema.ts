...
export const handoverPackages = sqliteTable('handover_packages', {
  ...
  status: text('status', {
    enum: ['draft', 'in_review', 'ready_to_issue', 'issued', 'accepted', 'revoked', 'expired'],
  ...
  acceptedSignatureDataUrl: text('accepted_signature_data_url'),
  ...
  snapshotJson: text('snapshot_json', { mode: 'json' }).$type<Record<string, unknown> | null>(),
  ...
  issuedByIdx: index('idx_handover_packages_issued_by').on(table.issuedBy),
  revokedByIdx: index('idx_handover_packages_revoked_by').on(table.revokedBy),
  ...
  issuedAtIdx: index('idx_handover_packages_issued_at').on(table.propertyId, table.issuedAt),
  expiresAtIdx: index('idx_handover_packages_expires_at').on(table.propertyId, table.expiresAt),
  packageHashUnique: uniqueIndex('handover_packages_package_hash_unique').on(table.packageHash),
  publicAccessTokenHashUnique: uniqueIndex('handover_packages_public_access_token_hash_unique').on(table.publicAccessTokenHash),
}));
...
export const serviceShareLinks = sqliteTable('service_share_links', {
  ...
  tenantIdx: index('idx_service_share_links_tenant').on(table.tenantId),
...
export const propertyAccessCredentials = sqliteTable('property_access_credentials', {
  ...
  tenantIdx: index('idx_property_access_credentials_tenant').on(table.tenantId),
  propertyIdx: index('idx_property_access_credentials_property').on(table.propertyId),
}));
...
export const pixCharges = sqliteTable('pix_charges', {
  ...
  tenantIdx: index('idx_pix_charges_tenant').on(table.tenantId),
...
export const nfeImports = sqliteTable('nfe_imports', {
  ...
  tenantIdx: index('idx_nfe_imports_tenant').on(table.tenantId),
