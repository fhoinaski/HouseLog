...
async function loadTenantServiceOrder(
  db: ReturnType<typeof getDb>,
  tenantId: string,
  propertyId: string,
  serviceId: string
): Promise<ServiceBidOrderContext | null> {
...
bids.get('/', async (c) => {
  ...
  if (!order) return err(c, 'OS ṇo encontrada', 'NOT_FOUND', 404);
  ...
  const results = await db
    ...
    .where(and(eq(serviceBids.tenantId, tenantId), eq(serviceBids.serviceId, serviceId)))
...
bids.post('/', async (c) => {
  ...
  const [existing] = await db
    ...
    .from(serviceBids)
    .where(and(eq(serviceBids.tenantId, tenantId), eq(serviceBids.serviceId, serviceId), eq(serviceBids.providerId, userId), eq(serviceBids.status, 'pending')))
    .limit(1);
  ...
  const [bid] = await db
    ...
    .from(serviceBids)
    .innerJoin(users, eq(users.id, serviceBids.providerId))
    .where(and(eq(serviceBids.id, id), eq(serviceBids.tenantId, tenantId), eq(serviceBids.serviceId, serviceId)))
    .limit(1);
...
bids.patch('/:bidId/status', async (c) => {
  ...
  const [bid] = await db
    ...
    .from(serviceBids)
    .where(and(eq(serviceBids.id, bidId), eq(serviceBids.tenantId, tenantId), eq(serviceBids.serviceId, serviceId)))
  ...
  if (body.status === 'accepted') {
    ...
    await db.run(sql`UPDATE service_bids SET status = 'rejected', updated_at = datetime('now') WHERE tenant_id = ${tenantId} AND service_id = ${serviceId} AND id != ${bidId} AND status = 'pending'`);
  }
