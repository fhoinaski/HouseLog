...
- **Auth/splash Calm OS full (2026-05-17)**: `require-auth.tsx` (loading state de sessao), `entry-shell.tsx` (focus ring + radius tokens) e `splash/page.tsx` (preview card interno, badge Estavel, metricas, status list, footer) totalmente migrados para tokens Calm OS. Nenhum token dark/tech restante nos componentes de auth/entrada. Footer atualizado de "Architectural Lens" para "HouseLog Calm OS". CVA variants (`visual-system.ts`): `cardVariants`, `buttonVariants`, `badgeVariants`, `chipVariants`, `actionTileIconVariants`, `documentRowVariants`, `documentTypeIconVariants`, `inventoryItemCardVariants`, `chatBubbleVariants`, `sensitiveFieldVariants`, `surfaceVariants`, `pageHeaderVariants` e `fieldVariants` todos migrados para tokens `--hl-*`, eliminando dependencia de tokens dark/tech herdados em qualquer Card, Badge, Chip, Button, ActionTile, DocumentRow, InventoryItem, ChatBubble e SensitiveField. Owner screens (`properties/page.tsx`, `properties/new/page.tsx`, `schedule/page.tsx`, `financial/page.tsx`, `settings/page.tsx`) e `premium-readonly-sections.tsx` (WarrantyCard, RenovationCard, ChecklistItemRow, MetricBox, InfoCell, handover packages) todos migrados.
...
- **Calm OS refinamento de paleta + bridge (2026-05-17)**: `defaultTheme` corrigido de `"dark"` para `"light"` em `theme-provider.tsx`, eliminando forcagem global da classe `.dark` sobre todos os componentes shadcn. Shadcn bridge em `@layer base :root` de `globals.css` repontado de tokens legados dark (`--bg-page`, `--card-bg`, `--text-tertiary`) para tokens `--hl-*`. Bloco `:root` unlayered adicionado ao final de `globals.css` com paleta refinada "warm premium + petroleo profundo + texto forte" (`--hl-bg: #f7f6f3`, `--hl-text: #1c1a17`, `--hl-primary: #1b4f6f`) + remapeamento de todos os tokens legados de texto (`--text-primary/secondary/tertiary`), fundo (`--bg-page/surface/subtle`, `--card-bg`, `--surface-base/muted/strong`) e borda (`--border-default/subtle/strong`) para valores light Calm OS — corrigindo `dashboard/page.tsx`, `properties/[id]/page.tsx` e outros sem necessidade de editar arquivo a arquivo. `page-header.tsx` e `page-section.tsx` migrados de `text-text-*` para `text-hl-*`. Foco ring (`--field-focus-ring`), nav text (`--nav-text-active/inactive`) e `.hl-calm-focus` corrigidos para tokens Calm OS. `.hl-error`, `.hl-hint`, `.hl-section-title` repontados para `--hl-danger` e `--hl-text-muted`. `TopNav`/`BottomNav` — `var(--radius-md)` substituido por `var(--hl-radius-control)`.
- **Provider settings Calm OS (2026-05-17)**: secao educacao/portfolio de `provider/settings/page.tsx` migrada; `var(--surface-strong/base)` substituidos por `bg-hl-surface-muted`/`bg-hl-surface`, `var(--radius-xl/lg)` por `var(--hl-radius-card/control)`, `text-text-secondary/tertiary` por `text-hl-text-muted`.
- **Risco residual (Calm OS)**: ainda existem telas legadas com tokens antigos nao cobertos pelo override global (principalmente sub-telas de OS/servico, documentos, handover, inventory, edit de imovel). Enquanto a migracao nao terminar, ha risco de inconsistencia visual residual nessas telas.
- **Recomendacao**:
