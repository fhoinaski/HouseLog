...
- `src/app/(app)/properties/[id]/services/page.tsx` e `service-requests/page.tsx`: mantem lista atual e adiciona toggle Lista/Kanban. `services/page.tsx` permite avancar OS por transicoes existentes do backend; `service-requests/page.tsx` usa Kanban visual/read-only para orcamentos.
